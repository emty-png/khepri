...
pub fn show_hierarchy(ui: &mut egui::Ui, scene: &mut Scene) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.painter().rect_filled(tab_rect, egui::CornerRadius::same(4), normal_bg);
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            if is_open {
                ...
                let popup_pos = egui::pos2(
                    btn_rect.right() - popup_width,
                    btn_rect.bottom() + 4.0,
                );
                let outer_rect = egui::Rect::from_min_size(popup_pos, egui::vec2(popup_width, popup_height));
                    ...
                    .show(ui.ctx(), |area_ui| {
                        ...
                        area_ui.scope_builder(egui::UiBuilder::new().max_rect(inner_rect), |ui| {
                            for shape in [ShapeType::Rectangle, ShapeType::Circle, ShapeType::Triangle] {
                                let item_text = egui::RichText::new(shape.name()).size(11.0).color(fg);
                ...
                if pointer
                    && !outer_rect.contains(ui.input(|i| i.pointer.interact_pos().unwrap_or_default()))
                    && !btn_rect.contains(ui.input(|i| i.pointer.interact_pos().unwrap_or_default()))
                {
                    ui.data_mut(|d| d.insert_temp(state_id, false));
                }
            }
